...
<html>
  <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <h2 style="margin: 0 0 12px 0;">Home Donation Confirmation</h2>
    ...
    @php
      ...
      $appointmentDate = $homeAppointment->appointment->appointment_date ?? null;
      $appointmentTime = $homeAppointment->appointment_time ?? null;
      $hospitalName = $homeAppointment->hospital->name ?? 'N/A';
    @endphp

    <p>Dear {{ $greetingName }},</p>

    <p>Your <strong>Home Donation</strong> registration has been received successfully.</p>
    ...
    <table style="width: 100%; border-collapse: collapse; margin: 10px 0;">
      ...
      <tr>
        <td style="padding: 6px 14px 6px 0; font-weight: bold; text-align: left; border-bottom: 1px solid #eee;">Address</td>
        <td style="padding: 6px 0; border-bottom: 1px solid #eee;">{{ $homeAppointment->address ?? ($donor->address ?? 'N/A') }}</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td style="padding: 6px 0; border-bottom: 1px solid #eee;">{{ $hospitalName }}</td>
      </tr>
      <tr>
        <td style="padding: 6px 14px 6px 0; font-weight: bold; text-align: left;">Emergency Contact</td>
        <td style="padding: 6px 0;">{{ $homeAppointment->emerg_contact ?? 'N/A' }} (+961 {{ $homeAppointment->emerg_phone ?? 'N/A' }})</td>
      </tr>
    </table>
